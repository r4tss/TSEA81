...
lift_type lift_create(void) 
{
	...
	{
		pthread_cond_init(&lift->change_exit[i], NULL);
		pthread_cond_init(&lift->change_enter[i], NULL);
	}
...
void lift_has_arrived(lift_type lift)
{
	...
	pthread_cond_broadcast(&lift->change_exit[lift->floor]);

	//printf("Persons leaving: %i, Persons entering: %i\n", passengers_leaving(lift), persons_entering(lift));

	while(passengers_leaving(lift) > 0)
	{
		pthread_cond_wait(&lift->change, &lift->mutex);
	}

	pthread_cond_broadcast(&lift->change_enter[lift->floor]);

	while(persons_entering(lift) > 0 && n_passengers_in_lift(lift) < MAX_N_PASSENGERS)
	{
		pthread_cond_wait(&lift->change, &lift->mutex);
	}
...
static int passenger_wait_to_leave(lift_type lift, int wait_floor)
{
    int waiting_ready =
        /* the lift is not moving */ 
        !lift->moving && 
...
void lift_travel(lift_type lift, int id, int from_floor, int to_floor)
{
	...
	{
		pthread_cond_wait(&lift->change_enter[from_floor], &lift->mutex);
	}
	...
	{
		pthread_cond_wait(&lift->change_exit[to_floor], &lift->mutex);
	}
