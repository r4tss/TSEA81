...
void draw_lift(lift_type lift)
{
  ...
  if (lift->moving == 1)
    {
      si_ui_draw_image("lift_red", xLift, yLift[lift->floor]);
    }
  else
    {
      si_ui_draw_image("lift_green", xLift, yLift[lift->floor]);
    }
#else
  si_ui_draw_image("lift_red", xLift, yLift[lift->floor]);
#endif
  ...
  for (i = 0; i < MAX_N_PASSENGERS; i++)
    {
      if (lift->passengers_in_lift[i].id != NO_ID)
        {
#ifdef NEWGUI
 	  if (lift->passengers_in_lift[i].to_floor == 0)
	    {
	      si_ui_draw_image("D0", 
			       xLift + i*cxGubbe, yLift[lift->floor] - cyGubbe + delta_y_Gubbe);
	    }
 	  if (lift->passengers_in_lift[i].to_floor == 1)
	    {
	      si_ui_draw_image("D1", 
			       xLift + i*cxGubbe, yLift[lift->floor] - cyGubbe + delta_y_Gubbe);
	    }
 	  if (lift->passengers_in_lift[i].to_floor == 2)
	    {
	      si_ui_draw_image("D2", 
			       xLift + i*cxGubbe, yLift[lift->floor] - cyGubbe + delta_y_Gubbe);
	    }
 	  if (lift->passengers_in_lift[i].to_floor == 3)
	    {
	      si_ui_draw_image("D3", 
			       xLift + i*cxGubbe, yLift[lift->floor] - cyGubbe + delta_y_Gubbe);
	    }
 	  if (lift->passengers_in_lift[i].to_floor == 4)
	    {
	      si_ui_draw_image("D4", 
			       xLift + i*cxGubbe, yLift[lift->floor] - cyGubbe + delta_y_Gubbe);
	    }
	  ...
	  si_ui_draw_string(message, xLift + i*cxGubbe + id_x_offset, 
                            yLift[lift->floor] - cyGubbe + delta_y_Gubbe + id_y_offset + 15);
        }
    }
  ...
  for (floor = 0; floor < N_FLOORS; floor++)
    {
	  ...
	  if (lift->persons_to_enter[floor][i].id != NO_ID)
            {
#ifdef NEWGUI
	      if (lift->persons_to_enter[floor][i].to_floor == 0)
		{
		  si_ui_draw_image("D0", xLift - LEVEL_OFFSET - i*(cxGubbe + 2), 
				   yLift[floor] - cyGubbe);
		}
	      if (lift->persons_to_enter[floor][i].to_floor == 1)
		{
		  si_ui_draw_image("D1", xLift - LEVEL_OFFSET - i*(cxGubbe + 2), 
				   yLift[floor] - cyGubbe);
		}
	      if (lift->persons_to_enter[floor][i].to_floor == 2)
		{
		  si_ui_draw_image("D2", xLift - LEVEL_OFFSET - i*(cxGubbe + 2), 
				   yLift[floor] - cyGubbe);
		}
	      if (lift->persons_to_enter[floor][i].to_floor == 3)
		{
		  si_ui_draw_image("D3", xLift - LEVEL_OFFSET - i*(cxGubbe + 2), 
				   yLift[floor] - cyGubbe);
		}
	      if (lift->persons_to_enter[floor][i].to_floor == 4)
		{
		  si_ui_draw_image("D4", xLift - LEVEL_OFFSET - i*(cxGubbe + 2), 
				   yLift[floor] - cyGubbe);
		}

	      sprintf(message, "   %d",
		      lift->persons_to_enter[floor][i].id); 
#else
